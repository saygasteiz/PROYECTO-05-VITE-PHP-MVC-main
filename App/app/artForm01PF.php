...
if(!isset($_POST['terminos']) || comprobarVacio($_POST['terminos'])){
    ...
    header("location:/es/showroom-pf?error=aceptar&campo=condiciones&nombre=$nombre&tel=$telefono&email=$email&mensaje=$mensaje#artForm01");
    die;
}else{
...
//     // Como viene vacía, redirijo a la página de contacto
//     // echo "Hay un error pues no ha aceptado las condiciones de privacidad";
//     header('location:/es/showroom-pf?error=condiciones');
//     die;
// }else{
//     $terminos = $_POST['terminos'];
// }
...
// que venga vacío
if(!isset($respUser)){
    header("location:/es/showroom-pf?error=vacio&campo=captcha&nombre=$nombre&tel=$telefono&email=$email&mensaje=$mensaje#artForm01");
    die;  
}
// que la respuesta sea errónea
if($respUser != $respSystem){
    header("location:/es/showroom-pf?error=error&campo=captcha&nombre=$nombre&tel=$telefono&email=$email&mensaje=$mensaje#artForm01");
    die; 
}
...
if(comprobarVacio($nombre)){
    header("location:/es/showroom-pf?error=vacio&campo=nombre&nombre=$nombre&tel=$telefono&email=$email&mensaje=$mensaje#artForm01");
    die;
}
// // que nombre sea menor de 3 y mayor de 40, salimos
if(comprobarCaracteres($nombre, 3, 40)){
    header("location:/es/showroom-pf?error=caracteres&campo=nombre&nombre=$nombre&tel=$telefono&email=$email&mensaje=$mensaje#artForm01");
    die; 
}

// que teléfono venga vacío, salimos
if(comprobarVacio($telefono)){
    header("location:/es/showroom-pf?error=vacio&campo=telefono&nombre=$nombre&tel=$telefono&email=$email&mensaje=$mensaje#artForm01");
    die; 
}

// que correo venga vacío, salimos
if(comprobarVacio($email)){
    header("location:/es/showroom-pf?error=vacio&campo=email&nombre=$nombre&tel=$telefono&email=$email&mensaje=$mensaje#artForm01");
    die; 
}

// // que correo no tenga la forma de un correo, salimos (expresión regular)
if(!comprobarEmailSintaxis($email)){
    header("location:/es/showroom-pf?error=sintaxis&campo=email&nombre=$nombre&tel=$telefono&email=$email&mensaje=$mensaje#artForm01");
    die;
}

// que mensaje venga vacío, salimos
if(comprobarVacio($mensaje)){
    header("location:/es/showroom-pf?error=vacio&campo=mensaje&nombre=$nombre&tel=$telefono&email=$email&mensaje=$mensaje#artForm01");
    die; 
}
// // que mensaje sea menor de 4 y mayor de 200, salimos
if(comprobarCaracteres($mensaje, 4, 200)){
    header("location:/es/showroom-pf?error=caracteres&campo=mensaje&nombre=$nombre&tel=$telefono&email=$email&mensaje=$mensaje#artForm01");
    die; 
}
...
header("location:/es/showroom-pf?envio=ok&nombre=$nombreUrl#artForm01");
die;
